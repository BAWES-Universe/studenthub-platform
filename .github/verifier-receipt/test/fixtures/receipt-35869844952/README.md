...
## What it is a fixture for now: the predicate, not just the recorded flag

The consumer no longer reads `provenance.admissible_as_pin`. It RE-DERIVES admissibility from this body with
`.github/verifier-receipt/admissibility.mjs`, the one module the emitter and the workflow's gate and guard
also call, and refuses any disagreement between the derivation and the flag. So this file now stands for two
cases rather than one:

* as it is, the derivation refuses it and the first derived reason is word for word the reason the receipt
  itself recorded - `"this receipt's verdict is failure, so there is nothing in it for a manifest to pin"`;
* with **only** `provenance.admissible_as_pin` flipped to `true` and its reason list emptied - the mutant the
  test builds in memory, never on disk - the derivation still refuses it, because the body still records
  `conclusion.verdict: "failure"`, `suite.state: "red"`, 60 failing tests, 8 skips it does not name and 132
  named tests the runner reported no location for. That is the forgery that once walked past three readers
  which each compared a computed boolean with itself.

Everything that derivation reads is present in the trimmed file verbatim (`conclusion`, `suite`,
`named_tests`, `candidate.authority_identity`, `workflow.*`); none of the three removed keys is read by it.
