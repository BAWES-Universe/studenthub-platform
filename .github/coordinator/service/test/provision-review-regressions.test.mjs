...
for (const [name,label,from,to,check] of mutants) test(`SHU71 review named mutation killed: ${name}`, async t => {
  ...
  for (const dep of ['host-suite-contract','shu71-production','shu71-runtime-schema','units']) text=text.replace(`'./${dep}.mjs'`,JSON.stringify(new URL('../'+dep+'.mjs',import.meta.url).href));
