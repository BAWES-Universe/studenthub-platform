...
test('B1_SINGLE_LANE: production credential drives coordinator build BLOCK revision re-review', async t => {
  ...
  assert.equal(effects(c.p, cleanupStart), 75, 'B1_RESUME_EFFECT_COUNT');
  ...
  assert.equal(effects(c.p, repeatStart), 6, 'B1_REVOKE_OBSERVATION_ONLY');
});
...
test('B1_TWO_LANES: B completes while A revision remains running, then A completes', async t => {
  ...
  assert.equal(effects(c.p, cleanupStart), 79, 'B1_EXPIRY_EFFECT_COUNT');
