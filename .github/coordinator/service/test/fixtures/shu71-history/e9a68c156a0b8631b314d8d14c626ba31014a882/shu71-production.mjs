...
export function createShu71Production(id, b = shu71Boundary) {
  ...
  function privateRead(file, uid = 0) {
    const fd = f.openSync(file, C.O_RDONLY | C.O_NOFOLLOW | C.O_NONBLOCK);
    try {
      const s = f.fstatSync(fd);
      need(s.isFile() && s.nlink === 1 && s.uid === uid && !(s.mode & 0o077) && s.size <= 4 * 1024 * 1024, 'ACT_FILE_CUSTODY');
    ...
    try {
      ...
      await step('sign', async () => {
        ...
        if (!signed) {
          ...
          const key = privateRead('/etc/shu/keys/shu71-signing.pem');
      ...
      const code = ['ACT_ID_OR_EXPIRY_INVALID', 'ACT_SIGNING_AMBIGUOUS', 'ACT_REF_BINDING', 'ACT_REVISION_BINDING',
