...
function configuration(spec) {
  ...
  check('SHU251_LIFECYCLE_SPEC', keys(c, ['activation_id', 'approval_sha256', 'approved_tree', 'identity', 'environment', 'directories',
    'systemd_version', 'capabilities', 'evidence_root', 'evidence_dir', 'rendered_sha256', 'checkout_before']) &&
  ...
  check('SHU251_LIFECYCLE_PATHS', absolute(c.evidence_root) && c.evidence_dir === `${c.evidence_root}/${c.activation_id}` &&
    ...
    equal(c.environment.supervisor, { path: spec.render.supervisorEnvironmentFile, uid: 0, gid: 0, mode: 0o600, kind: 'file' }) &&
  ...
  checkoutShape(c.checkout_before);
  check('SHU251_CHECKOUT_BASELINE', c.checkout_before.clean === true);
  return c;
}
export function approvedCheckout(spec) {
  return { sha: spec.window.approved_sha, head_ref: 'refs/heads/main', main: spec.window.approved_sha,
    origin_main: spec.window.approved_sha, tree: spec.lifecycle.approved_tree, clean: true };
}
function checkoutShape(v) {
  check('SHU251_CHECKOUT_TUPLE', keys(v, ['sha', 'head_ref', 'main', 'origin_main', 'tree', 'clean']) &&
    [v.sha, v.main, v.origin_main, v.tree].every(sha) && [null, 'refs/heads/main'].includes(v.head_ref) &&
...
async function preflight(spec, host, recovery = false, initialPin = false, observing = false) {
  ...
  check('SHU251_LIFECYCLE_CHECKOUT', initialPin
    ...
    : equal(p.checkout, { sha: spec.window.approved_sha, tree: c.approved_tree, clean: true }));
  const approved = approvedCheckout(spec);
  const interrupted = { ...c.checkout_before, sha: approved.sha, tree: approved.tree, head_ref: null };
  check('SHU251_CHECKOUT_BASELINE', initialPin
    ? [c.checkout_before, approved, interrupted, { ...approved, head_ref: null }].some(v => equal(p.checkout_tuple, v))
    : equal(p.checkout_tuple, approved));
...
function snapshotShape(s, spec) {
  ...
  check('SHU251_LIFECYCLE_SNAPSHOT', keys(s, ['files', 'enabled', 'active', 'pin', 'checkout']) && keys(s.files, TARGETS) && Object.entries(s.files).every(([n, f]) => DROP_IN_DIRECTORIES.includes(n)
...
function journalValid(j, spec) {
  check('SHU251_LIFECYCLE_JOURNAL', keys(j, ['version', 'binding', 'prior', 'entries', 'receipts', 'restart', 'rolled_back', 'cleanup_failures']) &&
    j.version === 1 && equal(j.binding, binding(spec)) && Array.isArray(j.entries) && Array.isArray(j.receipts) &&
    typeof j.rolled_back === 'boolean' && Array.isArray(j.cleanup_failures) && j.cleanup_failures.every(e => keys(e, ['code', 'entry']) && /^SHU251_[A-Z_]+$/.test(e.code) && Number.isSafeInteger(e.entry)) && j.entries.every((e, i) => keys(e, ['id', 'verb', 'target', 'before', 'after', 'status']) &&
      ...
      (e.verb === 'place' && TARGETS.includes(e.target) || ['enable', 'start'].includes(e.verb) && UNIT_NAMES.includes(e.target) ||
       e.verb === 'checkout' && e.target === spec.window.repo_dir || e.verb === 'daemon-reload' && e.target === null || e.verb === 'pin' && e.target === j.prior?.pin?.ref)) &&
  ...
  for (const e of j.entries) {
    ...
    check('SHU251_LIFECYCLE_INTENT', (e.verb === 'enable' ? ['disabled', 'masked', 'static', 'indirect', 'not-found'].includes(e.before) : equal(e.before, expected)) &&
      (e.verb === 'place' ? DROP_IN_DIRECTORIES.includes(e.target) ? equal(e.after, { kind: 'directory', uid: 0, gid: 0, mode: 0o755 }) : keys(e.after, ['kind', 'data', 'mode', 'uid', 'gid']) && e.after.kind === 'file' &&
        ...
        : e.verb === 'checkout' ? equal(e.after, approvedCheckout(spec)) || equal(e.after, j.prior.checkout)
    ...
    if (e.verb === 'checkout') replay.checkout = copy(e.after);
  }
...
function value(s, verb, target) {
  ...
  if (verb === 'checkout') return s.checkout;
  return null;
}
const errors = { place: 'SHU251_LIFECYCLE_PLACE', 'daemon-reload': 'SHU251_LIFECYCLE_RELOAD', enable: 'SHU251_LIFECYCLE_ENABLE',
  checkout: 'SHU251_CHECKOUT_EFFECT', start: 'SHU251_LIFECYCLE_START', pin: 'SHU251_LIFECYCLE_PIN' };
...
async function serviceReady(spec, host) {
  ...
  const r = await host.serviceReadiness();
  check('SHU251_LIFECYCLE_READINESS', keys(r, ['identity', 'listeners', 'supervisor', 'coordinator', 'committed_dispatch', 'runtime_dispatch', 'invocation_id']) &&
    ...
    s.active['shu-supervisor.service'] === 'active' && s.active['shu-coordinator.timer'] === 'active' &&
...
async function execute(step, spec, options, io) {
  ...
  check('SHU251_LIFECYCLE_IO', host && ['initialize', 'authorize', 'recordPreflight', 'resumeReceipt', 'finalize', 'probe', 'snapshot', 'stage', 'readiness', 'serviceReadiness', 'runningGateOff', 'withLock', 'load', 'save', 'place', 'systemd', 'pin', 'checkout', 'remoteMain'].every(k => typeof host[k] === 'function'));
  await host.initialize(step);
  if (step === 'preflight') {
    await host.authorize(step, null);
    const after = await preflight(spec, host);
    const result = receipt(step, spec, { binding: 'host_preflight', ok: true, ...pickBinding(spec), installed_sha256: {}, before: {}, after,
      journal_sha256: hash('null'), disposition: 'observed' });
    await boundary(host, 'SHU251_EVIDENCE_ARCHIVE', () => host.recordPreflight(result));
    return result;
  }
  ...
  return host.withLock(async () => {
    ...
    if (j === null) {
      ...
      check('SHU251_CHECKOUT_BASELINE', equal(prior.checkout, spec.lifecycle.checkout_before));
      j = { version: 1, binding: binding(spec), prior, entries: [], receipts: [], restart: null, rolled_back: false, cleanup_failures: [] };
    ...
    async function perform(e) {
      ...
      check('SHU251_LIFECYCLE_SUBSTITUTION', e.verb === 'checkout' || e.verb === 'daemon-reload' || equal(v, e.before) || equal(v, e.after));
      if (e.verb === 'checkout' || e.verb === 'daemon-reload' || e.verb === 'start' && e.target === 'shu-coordinator.service' || !equal(v, e.after)) {
        await boundary(host, errors[e.verb], () => e.verb === 'place' ? host.place(e.target, copy(e.before), copy(e.after))
          : e.verb === 'checkout' ? host.checkout(copy(e.before), copy(e.after))
    ...
    check('SHU251_LIFECYCLE_ORDER', (!j.rolled_back || ['host-rollback', 'pin-restore', 'pin-retain'].includes(step)) &&
      ...
      (!['readiness', 'running-gate-off', 'restart'].includes(step) || j.receipts.some(r => r.step === 'start') && !j.entries.some(e => e.status === 'pending')));
    ...
    if (step === 'install') {
      ...
      const proof = await host.runningGateOff();
      check('SHU251_RUNNING_GATE_OFF', keys(proof, ['before', 'after', 'ticks', 'writes', 'launches']) &&
        proof.ticks === 3 && proof.writes === 0 && proof.launches === 0 && equal(proof.before, proof.after));
      after = { service: await serviceReady(spec, host), proof };
    }
      ...
      j.cleanup_failures = [];
      const failures = [];
      const attempt = async (entry, fn) => {
        try { await fn(); } catch (error) {
          const code = /^SHU251_[A-Z_]+$/.test(error.code) ? error.code : 'SHU251_LIFECYCLE_ROLLBACK';
          failures.push({ code, entry });
        }
      };
      ...
      for (const e of [...j.entries].reverse()) {
        if (e.status === 'undone' || e.verb === 'pin' || e.verb === 'checkout') continue;
        await attempt(e.id, async () => {
      ...
      await attempt(-1, () => boundary(host, 'SHU251_LIFECYCLE_ROLLBACK', () => host.systemd('daemon-reload', null)));
      await attempt(-2, async () => {
        after = snapshotShape(await host.snapshot(), spec);
        check('SHU251_LIFECYCLE_ROLLBACK_EQUALITY', equal({ ...after, pin: j.prior.pin, checkout: j.prior.checkout }, j.prior));
      });
      j.cleanup_failures = failures;
      await save();
      if (failures.length) {
        // Preserve the first named refusal while returning all bounded failures.
        const error = Object.assign(new Error(failures[0].code), { code: failures[0].code, failures: copy(failures) });
        throw error;
      }
      j.rolled_back = true; await save();
    } else if (step === 'pin') {
      await change('checkout', spec.window.repo_dir, approvedCheckout(spec));
      await change('pin', j.prior.pin.ref, spec.window.approved_sha);
      after = { ...snapshotShape(await host.snapshot(), spec), approved_main: await host.remoteMain() };
    } else if (step === 'pin-restore') {
      check('SHU251_CHECKOUT_RESTORE', equal(before.checkout, approvedCheckout(spec)) || equal(before.checkout, j.prior.checkout) ||
        j.entries.some(e => e.verb === 'checkout' && e.status === 'pending' && equal(e.after, j.prior.checkout)));
      check('SHU251_LIFECYCLE_PIN_RESTORE', j.rolled_back && equal({ ...before, pin: j.prior.pin, checkout: j.prior.checkout }, j.prior) &&
        (before.pin.sha === spec.window.approved_sha || before.pin.sha === j.prior.pin.sha));
      await change('checkout', spec.window.repo_dir, j.prior.checkout);
      await change('pin', j.prior.pin.ref, j.prior.pin.sha);
      after = snapshotShape(await host.snapshot(), spec);
      check('SHU251_LIFECYCLE_ROLLBACK_EQUALITY', equal(after, j.prior));
      disposition = 'restored';
    } else if (step === 'pin-retain') {
      check('SHU251_LIFECYCLE_PIN_RETAIN', before.pin.sha === spec.window.approved_sha && equal(before.checkout, approvedCheckout(spec)) &&
        (!j.rolled_back || equal({ ...before, pin: j.prior.pin, checkout: j.prior.checkout }, j.prior)));
