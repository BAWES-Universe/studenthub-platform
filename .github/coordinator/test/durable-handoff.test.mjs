...
function harness(verdict = 'PASS', ready = false) {
  const comments = [receipt('codex-builder', 1, 'BUILD_READY'), receipt('claude-verifier', 2, verdict)]
    .map((r, i) => ({ body: coordinator.receiptCommentBody(r), createdAt: `2026-09-14T00:0${i}:00Z` }));
  ...
  const fetchImpl = async (url, opts) => {
    ...
    if (query.includes('CoordinatorIssues')) data = { issues: { nodes: [{ id: 'uuid-900', identifier: 'SHU-900', title: 'Handoff', state: { name: ready ? 'Todo' : 'In Progress' }, labels: { nodes: [{ name: 'repo:example/repo' }] }, relations: { nodes: [] } }] } };
      ...
      comments.push({ body: variables.body, createdAt: `2026-09-15T00:00:${String(++writes).padStart(2, '0')}Z` });
  ...
  const consume = async (options = {}) => {
    ...
    return coordinator.consumeDurableHandoffs({ receipts: read(), commentsByIssue: new Map([['SHU-900', structuredClone(comments)]]),
      ...
      config: { pilot_repo: 'example/repo' }, fetchImpl, stdout() {}, ...options });
  };
...
test('BLOCK-routes-to-writer', async () => {
  ...
  assert.equal(handoffContinuations(h.read()).get('SHU-900')?.successor.attempt_id, order.attempt_id, messages.block);
  ...
  try {
    ...
    writeFileSync(configPath, JSON.stringify({ pilot_repo: 'example/repo', enable_dispatch: true, max_dispatch: 2,
      repo_label_map: { 'repo:example/repo': 'example/repo' } }));
    ...
    assert.equal(handoffContinuations(remaining).size, 0, messages.claim);
...
test('unbacked-claim', async () => {
  ...
  assert.equal(handoffContinuations([vanished]).size, 0, messages.claim);
  ...
  assert.equal(handoffContinuations([...h.read(), claim]).size, 0, messages.claim);
...
test('resume-after-termination', async () => {
  ...
  try {
    ...
    completion.comments.push({ body: coordinator.receiptCommentBody(receipt('codex-builder', 3, 'BUILD_READY')), createdAt: '2026-09-14T00:03:00Z' });
