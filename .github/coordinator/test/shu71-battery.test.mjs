...
function finish(r, stage, lineage = []) {
  return foldLaunchOutcome(r, { stage: 'COMPLETED', external_run_id: `fixture_${r.attempt_id}`,
    ...
    callback: { attempt_id: r.attempt_id, target_sha: r.target_sha, result_sha: SHA_WRITE, stage,
      links: ['https://example.invalid/deterministic-defect-check'] } },
...
// Run in an isolated, capability-restricted vocabulary copy by the mutation harness.
if (process.env.SHU71_RESTRICTED === '1') test('SHU-71 restricted capability refusal', () => {
...
for (const verdict of ['BLOCKED', 'PASS']) {
  test(`SHU-71 reversal: Claude BUILD_READY then Codex ${verdict}`, () => {
    ...
    // Deterministic reviewer checks a concrete seeded boundary defect: zero is not positive.
    // Same observed reviewer/session and contract, differing only in the implementation reviewed.
    const implementation = verdict === 'BLOCKED' ? n => n >= 0 : n => n > 0;
    const actualVerdict = implementation(0) === false ? 'PASS' : 'BLOCKED';
...
function setup(t, multiple = false) {
  ...
  t.after(() => { supervisor.shutdown(); h.cleanup(); });
  ...
  const make = () => new DurableSupervisor({ stateDir: join(h.dir, 'supervisor'), secret: SECRET,
    probeProcess: () => probe, schedule: fn => scheduled.push(fn), spawnWorker: order => {
      const child = new EventEmitter(); child.pid = 10000 + children.length;
      child.processStartToken = `fixture-${child.pid}`; child.stdout = new EventEmitter(); child.stderr = new EventEmitter();
      child.kill = () => {}; child.executing = true; child.order = order;
...
test('SHU-71 responsiveness: reconcile returns during executing child', async t => {
  const f = setup(t); const start = performance.now(); await f.tick();
  ...
  t.diagnostic(`Measured reconcile return: ${elapsed.toFixed(2)}ms; simulated child executing before and after tick`);
});
test('SHU-71 recovery: duplicate wakeups launch once', async t => {
  const f = setup(t); await f.tick();
  const order = f.supervisor.store.readOrder(f.supervisor.store.attempts()[0]);
  await Promise.all(Array.from({ length: 4 }, () => f.supervisor.submit(signedSupervisorRequest(order, SECRET))));
  await f.drain(); await f.tick();
  assert.equal(f.children.length, 1, named.duplicate);
});
...
function isolated(t, changes, pattern, envExtra = {}) {
  ...
  for (const [file, before, after] of changes) {
    const target = join(dir, file), source = fs.readFileSync(target, 'utf8');
