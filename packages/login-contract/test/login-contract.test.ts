...
const CONTROL_MUTATIONS: ReadonlyArray<readonly [keyof ReferenceFaults, string, string]> = [
  ...
  ["preserveStateAfterBindingFailure", "state consumption after binding failure", LOGIN_CONTRACT_SCENARIOS[1]!],
  ["reusableState", "one-time state", LOGIN_CONTRACT_SCENARIOS[1]!],
  ["exchangeBeforeStateValidation", "callback validates state before code exchange", LOGIN_CONTRACT_SCENARIOS[1]!],
  ...
  ["wrongAuthorizationIssuer", "authorization request issuer", LOGIN_CONTRACT_SCENARIOS[0]!],
  ...
  ["wrongCallbackUrl", "exact callback URL", LOGIN_CONTRACT_SCENARIOS[0]!],
];
