...
let otherKeyPair: ReturnType<typeof generateEd25519KeyPair>;
...
describe("actor assertion v1 — acceptance corpus (SHU-0031)", () => {
  ...
  it("replayed jti fails closed on second use", async () => {
    ...
    const token = await signAssertion(claims(), keyPair.privateKeyPem);
    const first = await verifyAssertion(token, resolver, store, { expectedAudience: AUD });
    expect(first.ok).toBe(true);
    const second = await verifyAssertion(token, resolver, store, { expectedAudience: AUD });
  ...
  it("guest subject fails closed", async () => {
    ...
    expect(res.ok).toBe(false);
    expect(res.code).toBe(AssertionErrorCode.GUEST_SUBJECT);
  });
