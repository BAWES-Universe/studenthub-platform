...
Entries are ordered as shown. Child entries appear only when their own expression is satisfied.

| Effective grant/context | Primary navigation | Conditional/additive entries | Context-switch effect |
|---|---|---|---|
| `candidate:self` | Home (`U-01`), Profile (`C-01..05`), Discover (`C-06..07`), Invitations (`C-08`), Interviews (`C-09`), Work (`C-10..12`), Pay (`C-14..15`), Messages (`U-05`), Support (`U-06`) | Track (`C-11`) requires `assignment:active`; Certificates (`C-16`) appears when a record exists; Notifications (`U-04`) appears even at zero unread | Switching away removes all candidate-only entries; returning re-resolves current assignment and grants |
...
| `org:owner` at organization | `org:member` entries plus Edit organization (`O-03`), Team management (`O-04`), Store management (`O-05`), Notes (`O-06`) | Reports (`O-14`) only with an additional reporting/finance grant | Switching to a member-only context removes edit/manage actions and their endpoints immediately |
...
| `U-04` | `/notifications` — event feed | Notifications | authenticated principal with visible events | `notifications.self.list/read` | C3 / SHU-188 | Candidate activity/badges and app-shell polling |
...
| `A-13` | `/work/appeals`, `/work/corrections` | Work › Appeals | relevant staff work-review grant | `appeals.staff.*`, `work.staff.correct/reconcile` | W5 / SHU-173 (W6 dependency) | Appeal list/detail/status/contact/manual correction |
...
| `A-15` | `/access/accounts` | Access › Accounts | explicit account-admin capability | `accounts.list/view/invite/deactivate/recover` | I5 / SHU-154 | Admin/staff/inspector account CRUD minus excluded inspector identity |
...
| `A-30` | `/candidate-evaluations` | Recruiting › Evaluations | explicit staff evaluation capability | `candidate-evaluations.*` | SHU-234 | Admin/staff question bank, department reports and PDFs; named owning card because PR #77 found no parity slice |
