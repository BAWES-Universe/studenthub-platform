...
| Request provenance | `ip_address` (45) | Added for all five actor tables by `m240203_060625_block_ip.php:15`. `Candidate::beforeSave` derives the request IP (remote IP, or the first entry of `X-Forwarded-For`), writes it to this column, and on insert counts `candidate` rows sharing it created the same day, erroring above 10 — the signup throttle of §2.3 (`common/models/Candidate.php:1261-1295`). The `blocked_ip` table is a **separate** mechanism that does not read this column: `candidate/modules/v1/Module.php:69-75` derives the request IP the same way and tests it against `BlockedIp::find()`. The two share a derivation, not a data flow. Exposed by the staff projection — see F11 |
...
| Signup IP throttle | `beforeSave` `:1261-1295` (count and error at `:1285-1293`) | more than 10 `candidate` rows with the same `ip_address` and the same `DATE(candidate_created_at)` → validation error on insert |
...
| `candidate_uid` | **hidden from all five API projections** — the base `fields()` unsets it at `common/models/Candidate.php:1066-1070`, before any subclass receives `parent::fields()`, and the name appears nowhere in `staff/models/Candidate.php:22-38` or `admin/models/Candidate.php:19-34`. It is **not** a secret: the public verification app uses it as its unauthenticated route key (`/<candidate_uid>`, PD-07) and it is printed as the QR identity on ID cards, which is the basis of F10. "Hidden from the API projections" and "public on the card" are both true and must not be collapsed into either one | hidden | hidden | hidden | hidden |
...
- **Company search is a separate path and does not use the company projection.** `actionSearch` builds its query as `\staff\models\Candidate::find()` (`company/.../CandidateController.php:49-50`), and the effective staff projection removes only `candidate_uid`, `candidate_auth_key`, `candidate_password_hash` and `candidate_password_reset_token` — `staff/models/Candidate.php:22-38` unsets `candidate_auth_key` on top of a base `fields()` that has already removed the other three (`common/models/Candidate.php:1066-1070`) — so the response carries every other base-projection column, including email, phone, bank id, IBAN, account name, hourly rate, both timestamps, resume key, coordinates and `ip_address` (F11), so every row in employer search results serialises with the **staff** projection, not the company one. The per-store narrowing described above does not apply to that response. This is a disclosure path in its own right and is the reason PD-05 carries decision D1.
...
| PD-05 | Employer views a candidate | org-owner / recruiter, org scope | `GET v1/candidates/<id>` company app (`:691`), `search :25`, `list :83` | `view` uses the company projection §3 on any id; **`search` serialises with the staff projection** because it queries `\staff\models\Candidate` (`:49-50`), and the effective staff projection removes only `candidate_uid`, `candidate_auth_key`, `candidate_password_hash` and `candidate_password_reset_token` (`staff/models/Candidate.php:22-38` over `common/models/Candidate.php:1066-1070`) — so search returns `candidate_email`, `candidate_phone`, `bank_id`, `candidate_iban`, `bank_account_name`, `candidate_hourly_rate`, `candidate_created_at`, `candidate_updated_at`, `candidate_resume`, `candidate_latitude`, `candidate_longitude` and `ip_address`. Of these the company projection withholds the **first eight unconditionally** (`company/models/Candidate.php:23-47`, timestamps at `:26-27`, `bank` forced to `[]` at `:76`); it withholds `candidate_resume`, `candidate_latitude`, `candidate_longitude` and `ip_address` — and both civil ID photo keys — **only when the candidate's `store_id` is not one of the employer's managed stores** (`:49-64`), so for managed-store candidates those four are already visible through the company projection, as the §3 table states. See F11 | `company RequestCest` etc. 200-only (per GPT's sample) | REQUIRED, with decision D1 | S7 |
...
| **F11** | Employer **search** returns the full staff projection. `company/.../CandidateController.php:49-50` queries `\staff\models\Candidate`, and the effective staff projection removes only four fields: `staff/models/Candidate.php:22-38` unsets `candidate_auth_key` on top of `parent::fields()`, which has already removed `candidate_uid`, `candidate_password_hash` and `candidate_password_reset_token` (`common/models/Candidate.php:1066-1070`). Every other base-projection column is serialised, including `candidate_email`, `candidate_phone`, `bank_id`, `candidate_iban`, `bank_account_name`, `candidate_hourly_rate`, `candidate_created_at`, `candidate_updated_at`, `candidate_resume`, `candidate_latitude`, `candidate_longitude` and `ip_address` — and the candidate need not be in one of the employer's managed stores, because only the *projection* narrows by store, never the search. The company projection (`company/models/Candidate.php:18-77`) withholds email, phone, `bank_id`, IBAN, account name, hourly rate and both timestamps **unconditionally** (`:23-47`, timestamps at `:26-27`, `bank` forced to `[]` at `:76`), and withholds resume, coordinates, `ip_address` and the civil ID photo keys **only for candidates outside the employer's managed stores** (`:49-64`); employer search bypasses both tiers. An earlier draft named the staff-projection substitution without stating what it exposes, which understated it; a later one then overstated the contrast by implying the company projection withheld all of them unconditionally; the list itself also omitted `candidate_created_at` and `candidate_updated_at`, which the staff projection carries and the company projection unsets at `:26-27` | `company/.../CandidateController.php:49-50`; `staff/models/Candidate.php:22-38`; `common/models/Candidate.php:1066-1070`; `company/models/Candidate.php:23-47`, `:49-64` | **High**, legacy, live | Decision D1 must be taken against this list, not against the §3 company row. Platform S7 must serve employer search from an org-scoped projection, never a staff one |
...
|---|---|---|
| D1 | May employers see civil ID images, resume and coordinates of candidates in their stores? **And, per F11, what should employer *search* return** — today it returns the full staff projection including email, phone, IBAN, bank account name, hourly rate, resume key, coordinates and `ip_address`, for any candidate, managed store or not | Hide images and coordinates; keep resume behind authorized time-bounded retrieval; employer search returns an org-scoped projection only, never the staff one |
...
This is a **selected** list, not a complete inventory, and the mappings below are the migration that actually adds each column — verified by matching `addColumn\s*\(\s*['\"]candidate['\"]` at `c2ce255`, not by inferring from the migration's name. An earlier draft mislabelled three of them: `m211123_120542` is `candidate_time.php` and adds `candidate_preferred_time` rather than `candidate_pending_profile` (which is `m211223_081947_transfer_api.php:24`), and `candidate_intro` is `m230522_104706_tagline.php:15`, not `m230113_065332`, which is `candidate_evaluation.php` and adds no `candidate` columns at all. Migration filenames describe the feature, not the column, and must not be used as evidence of either.

`m170219_055954_missing_candidate_fields` (name_ar, birth_date, civil fields, hourly_rate; an earlier draft cited a nonexistent `m170219_151757`), `m170223_132254` (store_id), `m170303_134250` (approved), `m170306_112515` (bank_id, iban), `m170307_121642` (phone, bank_account_name), `m170420_125428` (university_id), `m170425_134445` (country_id), `m170427_123738` (personal_photo), `m170529_071050` (address_line1), `m200722_135609` (language), `m200724_100421` (new_email, email_verification, limit_email; nullable birth/civil/rate/auth_key), `m200729_094528` (driving_license, resume, gender, objective; creates skill and experience tables), `m200807_134023` (job_search), `m200907_135723` / `m201009_153032` / `m201019_103154` (video, processed, job id, webhook), `m200922_070412` (area, lat, long), `m201105_063330` (committed), `m201106_074724` (mom_kuwaiti), `m211123_120542_candidate_time` (candidate_preferred_time), `m211223_081947_transfer_api` (candidate_pending_profile), `m230406_110939` (profile_url), `m230522_104706_tagline` (candidate_intro), `m230504_083255` (tags), plus `is_duplicate`, `is_incomplete_profile`, `utm_uuid`, nullable password hash.
