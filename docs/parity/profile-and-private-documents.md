...
| PD-14 | Job-search status, committed | `POST job-search-status :679`, `GET :663`; committed via staff `toggle-committed` | `updateJobSearchStatus` sets `candidate_job_search_updated_at`; not-looking removes from Algolia (`Candidate.php:893-897`). **Committed is not a bare flag:** staff `actionToggleCommitted` (`staff/.../CandidateController.php:1040-1087`) accepts `note`, `type` and `candidate_id`, inserts a `Note` row, flips `candidate_committed` (`:1067`) and commits both **in one transaction** (`:1087`); when the toggle turns commitment **off** it sends `commitmentWarningEmail()` (`:1089-1090`). A staff-authored note is therefore a required input, not optional metadata | `tryToGetJobStatus`, `tryToUpdateJobStatus` assert value; `tryToToggleCommitted` | REQUIRED — the note, the transaction and the conditional email are all part of the contract | S2 |
...
|---|---|---|
| `candidate/tests/functional/AccountCest.php` | 43 | roughly one per account endpoint; almost all assert HTTP 200 plus `operation: success`; 12 assertions commented out; password tests assert messages; job-status tests assert the value |
