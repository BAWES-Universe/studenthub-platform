...
| Request provenance | `ip_address` (45) | Added for all five actor tables by `m240203_060625_block_ip.php:15`; feeds the signup throttle in §2.3 and the `blocked_ip` table. Exposed by the staff projection — see F11 |
...
| PD-05 | Employer views a candidate | org-owner / recruiter, org scope | `GET v1/candidates/<id>` company app (`:691`), `search :25`, `list :83` | `view` uses the company projection §3 on any id; **`search` serialises with the staff projection** because it queries `\staff\models\Candidate` (`:49-50`), and that projection unsets only `candidate_auth_key`, `candidate_password_hash` and `candidate_password_reset_token` — so search returns `candidate_email`, `candidate_phone`, `bank_id`, `candidate_iban`, `bank_account_name`, `candidate_hourly_rate`, `candidate_resume`, `candidate_latitude`, `candidate_longitude` and `ip_address`, every one of which the company projection deliberately withholds. See F11 | `company RequestCest` etc. 200-only (per GPT's sample) | REQUIRED, with decision D1 | S7 |
...
| **F11** | Employer **search** returns the full staff projection. `company/.../CandidateController.php:49-50` queries `\staff\models\Candidate`, and `staff/models/Candidate.php:22-38` unsets only `candidate_auth_key`, `candidate_password_hash` and `candidate_password_reset_token`. Every other column is serialised, including `candidate_email`, `candidate_phone`, `bank_id`, `candidate_iban`, `bank_account_name`, `candidate_hourly_rate`, `candidate_resume`, `candidate_latitude`, `candidate_longitude` and `ip_address` — and the candidate need not be in one of the employer's managed stores, because only the *projection* narrows by store, never the search. The company projection (`company/models/Candidate.php:18-77`) withholds email and phone unconditionally and forces `bank` to `[]`; the search path bypasses all of it. An earlier draft named the staff-projection substitution without stating what it exposes, which understated it | `company/.../CandidateController.php:49-50`; `staff/models/Candidate.php:22-38`; `company/models/Candidate.php:18-77` | **High**, legacy, live | Decision D1 must be taken against this list, not against the §3 company row. Platform S7 must serve employer search from an org-scoped projection, never a staff one |
