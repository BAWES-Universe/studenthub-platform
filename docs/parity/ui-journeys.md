...
| **FE-J2** | **Admin "limited access" is 123 active template conditions against two server checks.** `admin_limited_access` comes from the login response and hides create/edit/delete across admin, staff, inspector, company, candidate, transfer, reference-data, webhook, blocked-IP, currency and permission pages. Counted as occurrences at `c789b17`: **126 raw in `.html` under `src/app/pages`, of which 3 sit inside HTML comments and are inert, leaving 123 active gates** (`company/company-view/company-view.page.html:229`, `expense/expense-view/expense-view.page.html:11`, `staff/expenses/staff-expense-view/staff-expense-view.page.html:11`, all under `pages/logged-in/`). A further 15 active occurrences are component-side redirects in `.ts`, for 138 active of 141 raw across `src/app/pages`, and 150 active of 153 raw across `src/app`. Server-side, `grep -rn admin_limited_access admin/modules/v1/controllers admin/models` finds enforcement at exactly two places — `CandidateController.php:372` (delete) and `TransferController.php:2227` (invoice PDF before receipt) — plus the two setters in `AdminController.php:105,154`, the login echo at `AuthController.php:274`, and two **commented-out** checks (`AdminController.php:193`, `FulltimerController.php:123`). Every other mutation, including all transfer state changes, account CRUD and permission-section writes, accepts any bearer admin. A second gate, `restrictedAccess()`, unconditionally `return true` with its allow-list commented out (`auth.service.ts:562-571`), so the staff-salary and suggestion gates it guards are dead. | `admin` appendix ADMIN-FO-01..06 | server | **SHU-150 (I1)** |
...
| `admin:read-only` + `finance:read-settled` | The "limited admin" persona | 123 active page-template gates, 2 server checks (FE-J2) |
...
|---|---|---|---|
| **FE-J1/J2/J3** | Role and permission enforcement is a client-side concern in staff and admin, with two fail-open defaults and one inverted gate; the server has 2 checks against 130 active UI gates (7 staff permission keys plus 123 admin template gates) | **High** — every staff and admin capability boundary is currently advisory | SHU-150 owns the grant catalogue; treat the legacy permission tables as data to import, never as a model to port |
...
2. **FE-J20 unauthenticated `aws/config`** — that the authenticator is removed and not restored in all four app copies of `AwsController`. On the client side the precise position is: the four Angular clients (staff, admin, employer, mobile) call it with a bare `HttpClient` and send no bearer, and mobile calls it from an `APP_INITIALIZER` before login; the candidate web client *does* attach a bearer when Redux reports an authenticated session, because the call goes through the shared axios interceptor (`studenthub-candidate-react@2f0a121:src/providers/AxiosService.ts:20-34`) — but the endpoint does not require one, so the exposure is unchanged.
...
4. **FE-J2 count** — 123 *active* `admin_limited_access` gates in page templates (126 raw occurrences, 3 inside HTML comments) against exactly two server-side checks.
...
All seven were re-checked against the worktree at `c2ce255` and the five clones before publication, and three subagent citations were corrected in the process: the staff permission fetch-error path is fail-**closed** for gates (only the unlisted-company branch fails open), the admin gate figure was corrected from 129 to a measured count, and the candidate web client's `min_age`/`max_age` declarations are at `src/models/job.ts:22-23`.

Independent verification at head `0c4f2fe` then found two further factual errors, both corrected here and both worth recording because author re-verification did not catch them: the admin gate count conflated raw occurrences with active ones (126 raw, 123 active — three sit inside HTML comments), and the employer appendix row EMP-FO-30 stated `/aws/config` was "authenticated but unscoped", contradicting both the source and §4.4 of this document. The verifier also narrowed the client-side half of §11 claim 2, since candidate web does send a bearer the endpoint ignores. That is the second time a stated count in this document has been wrong in a way only an independent read caught, which is the argument for the `verifier:codex` reservation rather than against it.
