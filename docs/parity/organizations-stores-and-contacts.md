...
3. **A second hierarchy carries the money, and this cluster owns its parent side.** Billing does not run on `company.parent_company_id`; it runs on **`transfer.parent_transfer_id`**, added by `console/migrations/m170428_113508_invoice.php:9-15,24-26` — the same migration that **drops** the original `invoice` and `invoice_candidates` tables and adds `total` and `company_total` to `transfer`. A child transfer returns its **parent's** candidate lines filtered to its own `company_id` (`common/models/Transfer.php:521-535`); `TransferCandidate::getInvoiceNumber()` prefers the per-company child invoice and falls back to the parent's (`common/models/TransferCandidate.php:937-955`); and the rendered invoice resolves to the parent transfer, prints the **parent** transfer number, and shows `company_total` as the amount due (`admin/modules/v1/views/transfer/invoice.php:7,74,77`). Roll-ups already guard the resulting double-count with `parent_transfer_id IS NULL` (`admin/modules/v1/controllers/StatisticController.php:209,216,230,247`, the last commented *"ignore duplicate entries of child transfers"*, and `common/models/query/TransferCandidateQuery.php:202,219`). **That is one consolidated invoice with per-company subtotals, working in production today.**
...
| OR-34 | Consolidated invoicing across a company group | system, admin | `transfer.parent_transfer_id` lineage: `Transfer::getTransferCandidates` (`common/models/Transfer.php:521-535`), `TransferCandidate::getInvoiceNumber` (`:937-955`), `admin/.../views/transfer/invoice.php:7,74,77` | none | OTHER-CLUSTER (**SHU-185** owns invoice behaviour). Recorded here because the **organization hierarchy is a dependency of it**: consolidated total and per-company subtotals must reconcile without omitted or duplicated lines, and grouping must not confer cross-company access — see §1 fact 3 | — |
...
| D-OR4 | Currency rates have no source (OR-F6). Fix rates per contract, or integrate a rate provider? | Fix per contract — but **not** for the reason previously recorded here. Billing is not per-company: it is per **transfer group**, which spans companies (§1 fact 3), and `Transfer::beforeSave` inherits `currency_code` from the parent transfer before defaulting to `"KWD"` (`common/models/Transfer.php:181-196`), so currency belongs to the consolidation group rather than to each company independently. The answer may still be "fix per contract"; the premise that made it look obvious does not hold | Blocks nothing until multi-currency invoicing; wrong answer is expensive to unwind |
