...
perfectly still cannot do the job unless six further things are true, and each
one fails later, less visibly, and more expensively than a refusal at dispatch.

The coordinator therefore refuses to start a `codex-cli` worker until all six
