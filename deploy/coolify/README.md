...
2. Pin the resource to the exact reviewed Git commit. Set `SOURCE_REVISION` to the same full 40-character SHA.
...
- Roll back the gateway by redeploying the previously verified commit and setting `SOURCE_REVISION` to that SHA. Migrations are forward-only; do not delete or roll back the database volume. If a new migration is not backward compatible, restore from a separately verified database backup instead of reusing this application rollback procedure.
